...
def transmitSerial():
    ...
    for i in range(8):
        if i == 2 or i == 3 or i == 4 or i == 5:
            if (AOOData[i]!=-1):
                AOOData[i] = AOOData[i] * 10
            if (VOOData[i]!=-1):
                VOOData[i] = VOOData[i] * 10
            if (AAIData[i]!=-1):
                AAIData[i] = AAIData[i] * 10
            if (VVIData[i]!=-1):
                VVIData[i] = VVIData[i] * 10
            if (DOOData[i]!=-1):
                DOOData[i] = DOOData[i] * 10


    for i in range(8):
        AOOData[i] = int(AOOData[i])
        VOOData[i] = int(VOOData[i])
        AAIData[i] = int(AAIData[i])
        VVIData[i] = int(VVIData[i])
        DOOData[i] = int(DOOData[i])
    ...
    while True:
        ...
        if (failed == False):
            if (currentMode == 'AOO'):
                value = struct.pack('B' * 9, AOOData[0], AOOData[1], AOOData[2], AOOData[3], int(int(AOOData[7])/2), 1,0,0,0)
            elif (currentMode == 'VOO'):
                value = struct.pack('B' * 9, VOOData[0], VOOData[1], VOOData[4], VOOData[5],
                                    int(int(VOOData[6])/2),  2)
            elif (currentMode == 'AAI'):
                value = struct.pack('B' * 9, AAIData[0], AAIData[1], AAIData[2], AAIData[3],  int(int(AAIData[7])/2), 3,0,0,0)
            elif (currentMode == 'VOO'):
                value = struct.pack('B' * 9, VVIData[0], VVIData[1], VVIData[4], VVIData[5],
                                    int(int(VVIData[6])/2), 4,0,0,0)
            elif (currentMode == 'DOO'):
                value=struct.pack('B' * 9, DOOData[0], DOOData[1], DOOData[2], DOOData[3], DOOData[4], DOOData[5],
                                    int(int(DOOData[6])/2), int(int(DOOData[7])/2), 5,0,0,0)
            elif currentMode=='':
                value=struct.pack('B',123)
            try:
                print(value)
                print(currentMode)
                s1.write(value)
...
class DCMWindowsFunctionality:
    ...
    def updateInputModeInfo(self,windowName,lowerRateLimitInput,upperRateLimitInput,atrialAmplitudeInput,atrialPulseWidthInput,ventricularAmplitudeInput,ventricularPulseWidthInput,VRPInput,ARPInput):
        ...
        for index in range (8):#Loop to convert empty values to '-1' for the next time the program is run (will not work
            ...
            #     AOOData[index]=AOOData[index]*2
            #     VOOData[index] = VOOData[index] * 2
            #     AAIData[index] = AAIData[index] * 2
            #     VVIData[index] = VVIData[index] * 2
            #     DOOData[index] = DOOData[index] * 2
            if(AOOData[index]==''):
                AOOData[index]='-1'
            if (VOOData[index] == ''):
                VOOData[index] = '-1'
